...
class CRUDVendedores extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      user:{        
    ...
    this.onSubmitSearchVendedor = this.onSubmitSearchVendedor.bind(this);
  ...
  onSubmit = e => {
    ...
    if (name === '' || email === '' || password === '' || phone === '') {
      ...
      axios.post('/api/admin/signupv', this.state.user, {
        headers: {
  ...
  handleDelete(e,a){
    ...
    if (id === '') {
      ...
      axios.post('/api/admin/deletev', {id: id}, {
        headers: {
          'Content-Type': 'application/json'
        }
      })
  ...
  handleEdit(e,a){
    this.setState({
      emailVendedor:a.email
    },()=>{
      this.onSubmitSearchVendedor(e);
    })
    window.scroll(0,0);
  }
  ...
  onSubmitSearchVendedor = e => {
    const { email } = this.state.emailVendedor;

    e.preventDefault();
    if (email === '') {
      this.props.setAlert('Porfavor Ingree sus campos', 'danger');
    } else {
      axios.post('/api/admin/searchv', {email: this.state.emailVendedor }, {
        headers: {
          'Content-Type': 'application/json'
        }
      })
  ...
  onSubmitNewUser = e => {
    const { name, email, phone } = this.state.userNew;
    ...
    if (name === '' || email === '' || phone === '') {
      ...
      axios.post('/api/admin/updatev', this.state.userNew, {
        headers: {
          'Content-Type': 'application/json'
        }
      })
              ...
              <Card>
                ...
                <CardBody>
                  ...
                  <Row>
                      ...
                      <Col md="8">
                        <FormGroup>
                          <Input name="email" type="email" value={this.state.emailVendedor} onChange={this.onChangeEmail} required />
                        </FormGroup>
                      </Col>
                      <Col md="2">
                        <Button
                          ...
                          color="primary"
                          type="submit"
                          onClick={this.onSubmitSearchVendedor}
                        >
                          Buscar
                        </Button>
                      </Col>
...
export default connect(null, {loadUser, setAlert}) (CRUDVendedores);
